...
class LandlordRentTrackerController extends Controller
{
    ...
    public function getHistory(Request $request)
    {
        ...
        // ✅ Load stored reminders so they stay forever in landlord feed
        ...
        // ✅ Remove duplicates (current + stored)
        ...
        if ($now->gte($remind)) {
            $existingRemind = \App\Models\RentReminder::where('application_id', $application->id)
                ...
                ->when($groupFilter['type'] === 'null_or_zero', fn($q) => $q->whereNull('group_id'))
                ->where('type', 'reminder')
                ->where('for_date', $dueForDate)     // ✅ key by due date
                ->first();
    ...
    private function computeDue($app, int $month, int $year)
    {
        ...
        // ✅ start of day for consistent comparisons
