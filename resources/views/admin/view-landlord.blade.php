...
<div class="p-6 bg-white shadow rounded-lg max-w-5xl mx-auto">
    ...
    <div class="mt-8 flex gap-4">
        <form method="POST" action="{{ route('admin.accounts.suspend', ['type'=>'landlord','id'=>$landlord->id]) }}">
            @csrf
            <button class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Suspend Account</button>
        </form>

        <form method="POST" action="{{ route('admin.accounts.reactivate', ['type'=>'landlord','id'=>$landlord->id]) }}">
            @csrf
